...
// Compile and run it with: clang -I/opt/local/include/cairo create_icons.cpp -L/opt/local/lib/cairo -lcairo
...
#define HEIGHT 15
#define WIDTH 14
#define PATH "icons/windows/%s.png"
#endif
...
void create_pngs(int surfacewidth, int surfaceheight, float fontsize, const char* format, int start=0, int end=100)
{
    ...
    // cairo_text_extents(cr, "0123456789abcdefghijklmnopqrstuvwxyz", &extents);
    cairo_text_extents(cr, "0123456789abcdefghijklmnopqrstuvwxyz", &extents);

    float fontwidth = extents.width / 36; // average character width
    ...
    for (int i = start; i < end; i++)
    {
        ...
        char pathbuf[128];
        sprintf(pathbuf, format, buf);
        cairo_surface_write_to_png(surface, pathbuf);
    }
